...
treatment_std_desv = control_std_desv # standard deviation
...
x.field_names = ['trial run', 'n / treatment', 'Control - Average', 'Treatment - Average', 'Delta - average', 'Delta expected by customer?',
...
for i in range(n_trials_repetitions):
    ...
    x.add_row([i, n_samples_per_treatment, simul_control_average,  simul_treatment_average, delta, expected, round(ttest, 3), round(p, 3),
               treat_accepted])
